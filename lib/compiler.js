...
module.exports = function(breakdance, options) {
  ...
  return function(snapdragon) {
    ...
    var bullets = ['*', '-', '+'];
    var visit = this.compiler.visit;
    var before = breakdance.run('before', this.compiler);
    var after = breakdance.run('after', this.compiler);
    ...
    this.compiler.isInside = function(node, types) {
    ...
    this.compiler.visit = function(node, nodes, i) {
    ...
    this.compiler
      ...
      .set('p', function(node) {
        if (this.isInside(node, ['blockquote', 'li', 'table', 'dl'])) {
          node.newline = '';
        } else {
          node.newline = '\n';
        }
      ...
      .set('p.open', function(node) {
        this.emit(node.parent.newline + node.parent.newline);
      })
      .set('p.close', function(node) {
        this.emit(node.parent.newline);
      ...
      .set('code.open', function(node) {
        ...
        if (!opts().literalPre && util.isType(parent.parent, 'pre')) {
          ...
          this.emit(node.parent.prefix + node.parent.delim);
        }
      })
      ...
      .set('a.close', function(node) {
        ...
        var href = utils.resolveLink(node, 'href', this, opts());
        this.output = this.output.replace(/\s+$/, '');
        this.emit(']' + href);
      })
      ...
      .set('img', function(node) {
        ...
        if (attribs.src) {
          ...
          this.mapVisit(node);
          var src = utils.resolveLink(node, 'src', this, opts());
          this.emit(']' + src);
        }
      })
