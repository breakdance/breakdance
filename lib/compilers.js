...
module.exports = function(options) {
  ...
  return function(snapdragon) {
    ...
    var before = extend({}, plugins.before, options.before);
    var after = extend({}, plugins.after, options.after);
    ...
    snapdragon.compiler.visit = function(node, nodes, i) {
      ...
      var bef = before[node.type];
      var aft = after[node.type];
      var newNode;

      if (typeof bef === 'function') {
        bef.apply(snapdragon.compiler, arguments);
      }

      if (util.isOpen(node)) util.addType(state, node);
      if (util.isClose(node)) util.removeType(state, node);
      ...
      if (typeof aft === 'function') {
        return aft.call(snapdragon.compiler, newNode, nodes, i);
      }
      return newNode;
    };
    ...
    snapdragon.compiler
      ...
      .set('code.open', function(node) {
        ...
        if (!opts().literalPre && util.isType(parent.parent, 'pre')) {
          this.emit('\n\n', node);
          this.emit('```');
      ...
      .set('table', function(node) {
        ...
        util.mapVisit(node, {recurse: true}, function(tok) {
          if (tok.type === 'br') {
            tok.type = 'text';
            tok.val = '';
          }
      ...
      .set('tbody.open', function(node) {
        var table = util.last(state.tables);
      ...
      .set('a.close', function(node) {
        ...
        var href = utils.resolveLink(node, 'href', state, this.ast, opts());
        this.output = this.output.replace(/\s+$/, '');
        this.emit(']' + href, node);
      })
      ...
      .set('i', utils.emphasis('<i>'))
      .set('i.open', util.emit('<i>'))
      .set('i.close', util.emit('</i>'))

      /**
       * Bold (offset text conventionally styled in bold)
       */

      .set('b', utils.emphasis('<b>'))
      .set('b.open', util.emit('<b>'))
      .set('b.close', util.emit('</b>'))
      ...
      .set('img', function(node) {
        ...
        if (attribs.src) {
          ...
          this.mapVisit(node);
          var src = utils.resolveLink(node, 'src', state, this.ast, opts());
          this.emit(']' + src, node);
        }
      })
      ...
      .set('eos', function(node) {
        ...
        if (opt.reflinks) {
          this.output = utils.generateReflinks(this, state);
        }
