...
module.exports = function(options) {
  ...
  return function(snapdragon) {
    ...
    function isInside(node, types) {
      return helpers.isInside(state, node, types);
    }
    ...
    snapdragon.compiler
      ...
      .set('p', function(node) {
        node.newline = isInside(node, ['blockquote', 'li']) ? '' : '\n';
      ...
      .set('p.close', function(node) {
        ...
        if (isInside(node.parent, ['blockquote'])) {
          this.emit('\n');
        }
      })
      ...
      .set('code', function(node) {
        ...
        if (utils.isWrappingChar(ch) && !isInside(node, ['strong', 'em'])) {
          addPrefix = true;
        }
      ...
      .set('a.open', function(node) {
        ...
        if (/[)\]\w.]$/.test(this.output)) {
          this.emit(' ');
        } else if (/\*$/.test(this.output) && !isInside(node, ['strong'])) {
          this.emit(' ');
        } else if (/\_$/.test(this.output) && !isInside(node, ['em'])) {
          this.emit(' ');
        } else if (/\|$/.test(this.output) && isInside(node, 'table')) {
          this.emit(' ');
        } else {
      ...
      .set('em', function(node) {
        node.delim = isInside(node, 'em') ? '' : '_';
        this.mapVisit(node.nodes);
      })
      .set('em.open', function(node) {
        var ch = this.output.slice(-1);
        if (this.output && /[^\s\(\["]/.test(ch)) {
          if (!(isInside(node, 'strong') && ch === '*')) {
            this.emit(' ');
          }
        }
        this.emit(node.parent.delim, node);
      ...
      .set('strong', function(node) {
        node.delim = isInside(node, 'strong') ? '' : '**';
        this.mapVisit(node.nodes);
      })
      .set('strong.open', function(node, nodes, i) {
        var ch = this.output.slice(-1);
        if (this.output && /[^\s\(\[]/.test(ch)) {
          if (!(isInside(node, 'em') && ch === '_')) {
            this.emit(' ');
          }
        }
        this.emit(node.parent.delim, node);
      ...
      .set('input', helpers.noop)      // input control
    ...
    function normalize(node, state, options) {
      ...
      if (node.type === 'ul' || node.type === 'ol') {
        node.newline = isInside(node, 'table') ? '' : '\n';
        if (!state.lists.length) {
