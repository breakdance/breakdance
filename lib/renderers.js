  ...
  return function(snapdragon) {
    ...
    snapdragon.compiler
      ...
      .set('code', function(node) {
        ...
        node.text = node.text.split('`').join('&#x60');
      ...
      .set('em.open', function(node) {
        var prev = this.output.slice(-1);
        // if (/(^$|[\S\n])/.test(prev)) this.emit(' ', node);
        this.emit(node.parent.delim, node);
      })
      .set('em.close', function(node) {
        var isEmpty = node.prev.type === 'text' && !node.prev.val.trim();
        if (isEmpty && this.output.slice(-1) === '_') {
    ...
    function normalize(node, state, options) {
      if (options.whitespace === false) return;
      if (node.type === 'ul' || node.type === 'ol') {
        ...
        helpers.mapVisit(node.nodes, function(tok) {
          if (tok.type === 'text' && helpers.isNext(tok, ['ul', 'ol'])) {
            tok.val = tok.val.replace(/\s+$/, '');
          }
          if (tok.type === 'text') {
            if (!tok.val.trim()) tok.val = '';
          }
          return tok;
        });
      }
      ...
      if (node.type === 'blockquote') {
        helpers.mapVisit(node.nodes, function(tok) {
          if (tok.type === 'text') {
            if (!tok.prev) tok.val = utils.trimLeft(tok.val);
            if (!tok.val.trim()) tok.val = '';
          }
        });
      }
