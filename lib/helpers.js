...
/**
 * Returns true if `state` has already registered the given `type`.
 * Skips over text nodes.
 */

helpers.isAfterType = function(state, type) {
  if (Array.isArray(type)) {
    let types = type.slice();
    for (let i = 0; i < types.length; i++) {
      if (helpers.isAfterType(state, types[i])) {
        return true;
      }
    }
    return false;
