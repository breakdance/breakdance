...
function details(node) {
  // if (node.name === 'span' && node.attribs.class === 'toctoggle') {
  //   // console.log(node)
  // }
  // if (node.name === 'a' && /collapseButton/.test(node.attribs.id)) {
  //   console.log(node)
  // }
  if (/collapseButton/.test(node.attribs.id)) {
    node.parent.children[0].data = '';
    node.parent.children[node.parent.children.length - 1].data = '';
    var parent = $(node.parent).html();
    $(node.parent).html('<details>' + parent + '</details>');
  }
}
